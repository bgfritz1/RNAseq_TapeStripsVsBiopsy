...
The difference in number of genes with detectable expression above a given expression threshold was calclated for each tape-strip and biopsy in a pair and tested with a paired t-test. 
This was repeated for incremental increases in the threshold cutoff from 10 to 75. The remaining analysis was performed on *vst*-normalized (DEseq2) count matrix unless otherwise specified.[@RN138] 
...
Differential gene-expression was performed with DESeq2 on the filtered, un-normalized count matrix. Differentially expressed genes were identified as those with an 
...
5 non-AD controls (median age: 27; 60% female) (Table \@ref(tab:metadata-table), Table S1). Five of the seven AD patients had 
...
we first compared the total number of genes with detectable expression between each matched tape-strip and skin biopsy pair (Table S1,S3). 
...
tape-strip and skin biopsy samples, across all AD and non-AD samples. We observed an overall medium correlation 
...
gene-expression analysis between active AD skin and non-AD controls.
Tape-strips identified `r Ecz_n_tape_up + Ecz_n_tape_down` DEGs, where `r Ecz_n_tape_up` showed increased 
expression in active AD and `r Ecz_n_tape_down` showed increased expression in non-AD controls (Table S7). Comparatively, 
skin biopsies identified `r Ecz_n_biops_up + Ecz_n_biops_down` DEGs, where `r Ecz_n_biops_up` 
showed increased expression in active AD (Figure \@ref(fig:figure4)A) and `r Ecz_n_biops_down` showed increased expression in non-AD controls (Table S8). 
...
which showed significantly increased expression in non-AD controls (Figure \@ref(fig:figure4)C). Contrarily, biopsies identified several S100As (S100A7/S100A8/S100A9) and defensins (DEFB4A/DEFB4B), 
with increased expression in active AD. 

This analysis was repeated to compare inactive AD skin with non-AD controls. Interestingly, tape-strips identified more genes in this comparison than between active AD and non-AD controls (Figure \@ref(fig:figure4)B). 
...
showed increased expression in non-AD controls (Table S10). Comparatively, biopsies identified only `r prettyNum(AD_n_biops_up + AD_n_biops_down, big.mark = ",")` 
DEGs (Table S11). Biopsies identified several keratin-associated genes with increased 
...
Further, we compared the ability of tape-strips and biopsies to detect of bacterial mRNA (Figure \@ref(fig:figure5)). 
...
systematic biases between methods, and including paired analyses of active AD, inactive AD, and non-AD controls. 

We identified over 6,000 DEGs due to method across both AD and non-AD controls - a similar magnitude as observed 
...
This is highlighted in Figure \@ref(fig:figure2)B, where a subset of tape-strip samples from non-AD controls clustered closer to skin biopsies. 
It is unclear whether this effect is biological or technical. Further, the mean expression of genes identified as significantly differentially expressed between lesional-AD and non-AD controls
